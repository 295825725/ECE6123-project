...
image_list = glob.glob(opt.CACDDataset_path+"/*.jpg")
random.shuffle(image_list)
num_image = len(image_list)

num_train_image = int(num_image*0.2)
num_val_image = int(num_image*0.05)
num_test_image = int(num_image*0.05)#num_image-num_train_image-num_val_image
print(num_train_image,num_val_image,num_test_image)
def prepare_dataset(name, size, _image_list):

    with h5py.File(opt.Newdataset_path+"/CACD2000_{}.hdf5".format(name), 'w') as f:
